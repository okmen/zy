...
public class MD5Encrypt 
{
	...
	public static String encrypt(String source) {
		...
		try {
			...
			char result[] = new char[16 * 2];// MD5结果返回的是32位字符串，每位是16进制表示�?
			int k = 0;
			for (int i = 0; i < 16; i++) {// 循环16次，对每个字节进行操作转�?
				byte everyByte = hashCalc[i];
				System.out.println(everyByte);
				result[k++] = hexChar[everyByte >>> 4 & 0xf];// 对每个字节的�?4位进行处理，逻辑右移，再相与
				result[k++] = hexChar[everyByte & 0xf];// �?4位转�?
			}
			s = new String(result);
